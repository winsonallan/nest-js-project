...
import { getStatus, getWorkingDaysInMonth, toLocalDateString } from "@/lib/attendance";
import { inputStyle, PAGE_SIZE_OPTIONS, type SortDir } from "@/components/constants";
    ...
    <div className="space-y-4">
      ...
      {employee && (
        <div className="rounded-2xl p-4 flex flex-wrap gap-6"
          ...
          <div>
            <p className="text-xs mb-0.5" style={{ color: "var(--brownish-dark-grey)" }}>Status</p>
            <span className="text-xs px-2.5 py-1 rounded-full font-semibold"
              style={{
                background: employee.isActive ? "var(--lighter-grey-purple)" : "rgba(112,108,97,0.1)",
                color: employee.isActive ? "var(--grape-purple)" : "var(--brownish-dark-grey)",
              }}>
                ...
                return (
                  <tr key={item.id}
                    ...
                    <td className="px-4 py-3 text-xs" style={{ color: "var(--brownish-dark-grey)" }}>
                      {item.notes ?? <span style={{ color: "var(--light-transparent-grey)" }}>—</span>}
                    </td>
                    <td className="px-4 py-3">
                      {item.photoPath ? (
